...
<Cards>
  <Cards.Card title="Complete Example" href="#complete-example" />
  <Cards.Card title="Runtime Configuration" href="#runtime-configuration" />
  <Cards.Card title="Expert Definition" href="#expert-definition" />
  <Cards.Card title="Skill Definition" href="#skill-definition" />
</Cards>
...
<Callout type="info">
See [Providers and Models](/references/providers-and-models) for available models and environment variables.
</Callout>
...
| `network` | object | No | Network configuration for docker runtime |

### Network Configuration (Docker Runtime)

Configure network access restrictions for the docker runtime:

```toml
[experts."my-expert".network]
allowedDomains = ["api.github.com", "*.example.com"]
```

| Field | Type | Description |
|-------|------|-------------|
| `allowedDomains` | string[] | Domains the Expert can access (supports wildcards like `*.example.com`) |

<Callout type="info">
Network restrictions only apply when using `--runtime docker`. The final allowlist is merged from:
1. Global `network.allowedDomains`
2. Expert-level `network.allowedDomains`
3. Skill-level `allowedDomains`
4. Provider API domains (auto-included based on provider)
</Callout>
