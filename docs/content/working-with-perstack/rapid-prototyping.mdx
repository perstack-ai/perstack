...
See [Extending with Tools](/working-with-perstack/extending-with-tools) for more.
...
See [Taming Prompt Sprawl](/working-with-perstack/taming-prompt-sprawl) for the full pattern.

### Write effective instructions

How you write your instruction matters. Aim for the right level of specificity — not too rigid, not too vague.

See [Best Practices](/making-experts/best-practices) for guidelines on writing effective instructions, structuring your Experts, and common pitfalls to avoid.

## When your prototype grows

At some point, your prototype will need more:

- **External tools**: The agent needs to search the web, query a database, or call an API → See [Extending with Tools](/working-with-perstack/extending-with-tools)
- **Multiple responsibilities**: The prompt is getting long and the agent is getting confused → See [Taming Prompt Sprawl](/working-with-perstack/taming-prompt-sprawl)
- **Production deployment**: The prototype works and you want to ship it → See [Going to Production](/working-with-perstack/going-to-production)

The same TOML definition scales from prototype to production. You're not throwing away work — you're building on it.

## What's next

- [Making Experts](/making-experts) — Learn the full Expert definition format
- [Best Practices](/making-experts/best-practices) — Design patterns for effective Experts
