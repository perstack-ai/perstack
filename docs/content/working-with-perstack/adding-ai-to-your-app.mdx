...
See [Going to Production](/working-with-perstack/going-to-production) for the full Docker setup.
...
| Event | Meaning |
|-------|---------|
| `initializeRuntime` | Runtime initialized, Expert loaded |
| `generationStart` | LLM is generating a response |
| `generationComplete` | LLM finished generating |
| `toolCallStart` | Expert is calling a tool |
| `toolCallComplete` | Tool returned a result |
| `delegationStart` | Expert delegated to another Expert |
| `delegationComplete` | Delegate finished |
...
Checkpoints are saved to `workspace/perstack/runs/`. Each checkpoint contains the full execution state — you can resume from any checkpoint.
...
See [Running Experts](/using-experts/running-experts) for the embedding API.

## What's next

- [Going to Production](/working-with-perstack/going-to-production) — Docker setup and production deployment
- [Running Experts](/using-experts/running-experts) — CLI commands and runtime API
- [State Management](/using-experts/state-management) — Checkpoints, pausing, resuming
