...
For the full event schema, see [Runtime](/understanding-perstack/runtime#event-notification).
...
Checkpoints are saved to the workspace (see [Runtime](/understanding-perstack/runtime#the-perstack-directory)). Each checkpoint contains the full execution state — you can resume from any checkpoint.
