...
For the full execution model, including step counting and Coordinator vs. Delegated Expert differences, see [Runtime](/understanding-perstack/runtime).
...
**Note:** Interactive tools are only available to the Coordinator Expert. See [Why no interactive tools for delegates?](/understanding-perstack/experts#why-no-interactive-tools-for-delegates)
...
Events enable debugging, monitoring, and analysis. See [Runtime](/understanding-perstack/runtime#event-notification) for event types and the notification system.
