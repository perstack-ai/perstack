...
describe("@perstack/api-client: Studio", () => {
  ...
  describe("updateExpertJob", () => {
    it("should update an expert job", async () => {
      const client = new ApiV1Client({ baseUrl: "https://mock", apiKey: "test" })
      const input: UpdateExpertJobInput = {
        expertJobId: "expertjob123456789012345",
        status: "completed",
      }
      api.post(
        "/api/studio/v1/expert_jobs/expertjob123456789012345",
        {
          data: {
            expertJob: {
              ...expertJob,
      ...
      await expect(updateExpertJob(input, client)).resolves.toEqual({
        expertJob: {
          ...assertExpertJob,
          status: "completed",
        },
  ...
  describe("createCheckpoint", () => {
    it("should create a checkpoint", async () => {
      ...
      api.post(
        "/api/studio/v1/expert_jobs/expertjob123456789012345/checkpoints",
        {
          data: {
            checkpoint,
          },
      ...
      await expect(createCheckpoint(input, client)).resolves.toEqual({
