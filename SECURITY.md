...
- **Always use `--runtime docker`.** Default runtime has no isolation.
...
1. **Always use `--runtime docker`**
2. **Review `requiredEnv`** before providing API keys to Skills
3. **Minimize `allowedDomains`** — default to deny, add only what's needed
4. **Isolate workspace** — never mount directories containing secrets or credentials
5. **Sanitize stdout** — treat all output events as untrusted input

```bash
perstack run my-expert "query" --runtime docker
```
...
**Key principle:** The runtime doesn't enforce security boundaries — infrastructure does. This is sandbox-first design.
...
- Access the network (restricted with `--runtime docker`)
- Read sensitive files (restricted with `--runtime docker`)

**Mitigation:**
- Always use `--runtime docker`
...
| ------------- | ----------------------------------------------------------------------------- |
| `perstack`    | perstack.toml-level controls only (requiredEnv, pick/omit, context isolation) |
| `docker`      | Perstack on Docker (full sandbox + Squid proxy)                               |
| `cursor`      | Cursor's own security                                                         |
| `claude-code` | Claude Code's own security                                                    |
| `gemini`      | Gemini's own security                                                         |

**Note:** The security features described in this document (container isolation, Squid proxy, DNS rebinding protection) apply **only to Perstack on Docker** (`--runtime docker`). Other runtimes delegate security to their own implementations.
...
**Why `--runtime docker`?** It provides:
...
| Variable Category                          | Perstack on Docker | Perstack (default) | Notes                                        |
| ------------------------------------------ | ------------------ | ------------------ | -------------------------------------------- |
| `PATH, HOME, SHELL`                        | ✅                  | ✅                  | Required for process execution               |
| `TERM`                                     | ✅                  | ✅                  | Terminal compatibility                       |
| `NODE_PATH`                                | ✅                  | ✅                  | Node.js module resolution (⚠️ attack surface) |
| `HTTP_PROXY, HTTPS_PROXY`                  | ✅ (proxy enforced) | ❌ Not passed       | Docker only: forces proxy                    |
| `NO_PROXY, PERSTACK_PROXY_URL`             | ✅                  | ❌ Not passed       | Docker only                                  |
| `NPM_CONFIG_PROXY, NPM_CONFIG_HTTPS_PROXY` | ✅                  | ❌ Not passed       | Docker only: npm proxy                       |

**⚠️ NODE_PATH risk:** Included for compatibility, but allows skills to influence module resolution. In default runtime with no filesystem isolation, this increases attack surface.
...
>
> Windows lacks critical security primitives (`O_NOFOLLOW`, robust symlink restrictions). If you run untrusted Experts from the internet on Windows, **you MUST use `--runtime docker` via WSL2**.
>
> ```bash
> # Required for secure execution on Windows
> perstack run expert-name "query" --runtime docker
> ```
>
> Running untrusted Experts with the default runtime on Windows is **strongly discouraged** and may expose your system to arbitrary file access and code execution.
...
**Not prevented:** The command itself can still access files outside workspace (e.g., `cat /etc/passwd`), make network requests, or perform other operations allowed by the process. Use `--runtime docker` for actual isolation.
...
**Not prevented:** A malicious skill running in the same process can race against these checks or create symlinks. Use `--runtime docker` with read-only root filesystem for actual isolation.
